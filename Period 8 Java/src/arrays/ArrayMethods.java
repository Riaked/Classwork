...
public class ArrayMethods {
     ...
     * */
    	for (int i = 0; i < sortedArrayToSearch.length; i++){
    		for (int j = 0; j < sortedArrayToSearch.length-1; j++){
    			if ((sortedArrayToSearch[i] == key) && sortedArrayToSearch[j] > sortedArrayToSearch[j+1]){
        			return i;
        		}
    		}
    	}
    ...
    public static int longestSharedSequence(int[] array1, int[] array2){
        ...
        for (int seqStart = 0; seqStart < array1.length; seqStart++){
        	...
        	//resets the count after every seq has been checked
        	count = 0;
        }
        return max;
    }
	...
	//returns subarray cotaining elements in array1 from seqStart seqEnd
